...
#                "AptSetup failed with code %d" % ret)
        ...
        try:
            ...
            install_misc.chrex(self.target, 'rm', '-rf', '/etc/apt/sources.list.d')
            install_misc.chrex(self.target, 'rm', '-rf', '/etc/apt/preferences.d')
...
#            self.do_install(packages)
    ...
    def kylinpostaction_ghost(self):
        ...
        #kylin do postactions in /usr/share/ubiquity/kylin-postactions.sh grub
    ...
    def kylinpostaction(self):
        ...
        for kyfile in ('/cdrom/.kyinfo', '/cdrom/LICENSE'):
            ...
            if os.path.isfile(kyfile):
                if not misc.execute('rsync', '-a', kyfile, target_kyfile):
                    syslog.syslog(syslog.LOG_ERR,
                                  'Failed to copy installation kylin action file %s.' %(kyfile))
            else:
                os.system("rm -rf /target/etc/.kyinfo")
                os.system("rm -rf /target/etc/LICENSE")
        ...
        subprocess.call(['log-output', '-t', 'kylin-postactions-ubiquity', '/bin/bash', '/usr/share/ubiquity/kylin-postactions.sh'])
        #end kylin

        # kylin chroot do actions form cdrom
        if os.path.isfile('/cdrom/.kylin-post-actions'):
            os.system("/bin/cp /cdrom/.kylin-post-actions /target/bin/.kylin-post-actions")
            subprocess.call(['log-output', '-t', 'kylin-postactions-chroot', 'chroot', self.target,
                             '/bin/bash', '/bin/.kylin-post-actions', ])
        # end kylin chroot

        # kylin no chroot do actions from cdrom
        if os.path.isfile('/cdrom/.kylin-post-actions-nochroot'):
            subprocess.call(['log-output', '-t', 'kylin-postactions-nochroot', '/bin/bash', '/cdrom/.kylin-post-actions-nochroot'])
        # end kylin chroot
