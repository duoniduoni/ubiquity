...
class PageGtk(PageBase):
    ...
    # kylin add, click data partition callback function 
    ...
    # kylin add, click backup partition callback function 
    def on_backup_checkbutton_clicked(self, w):
        ...
        if self.backup_checkbutton.get_active():
            ...
            self.backup_checkbutton_factory.set_active(False)
        self.config.write(open('/tmp/kylin-data.ini', "w"))

    # kylin add, click backup partition callback function 
    def on_backup_checkbutton_factory_clicked(self, w):
        if not self.config.has_section("lixiang"):
            self.config.add_section("lixiang")
        if self.backup_checkbutton_factory.get_active():
            self.config.set("lixiang", "backup-factory", "1")
            self.backup_checkbutton.set_active(True)
        else:
            self.config.set("lixiang", "backup-factory", "0")
        self.config.write(open('/tmp/kylin-data.ini', "w"))
    ...
    # init show some info , from kylin add
    def set_checkbox_text(self, contents):
        ...
        if 'factory' in contents:
            self.backup_checkbutton_factory.set_label(contents['factory'].title)
            self.backuplabel_factory.set_markup(fmt % contents['factory'].desc)

    # Display prompt information in advanced partition interface according to different hardware
    def set_kylin_custom_text(self, contents):
    ...
    def set_autopartition_options(self, options, extra_options):
        ...
        ticked = False
        # Display the default state according to the preset value
        filename = '/tmp/kylin-data.ini'
        data_value = False
        backup_value = False
        backup_factory_value = False
        if os.path.isfile(filename):
            ...
            if self.config.has_section("lixiang"):
                ...
                if self.config.has_option("lixiang", "backup"):
                    ...
                    if tmp_value == "1":
                        backup_value = True
                if self.config.has_option("lixiang", "backup-factory"):
                    tmp_value = self.config.get("lixiang", "backup-factory")
                    if tmp_value == "1":
                        backup_value = True
        self.backup_checkbutton.set_active(backup_value)
        self.backup_checkbutton_factory.set_active(backup_factory_value)
...
#                    ticked = True
                ##### aarch64 and mips64el no show resize
                if self.arch_version == "aarch64" and  option == 'resize' and name == 'resize_use_free':
                    ...
                    # kobe
                    len_widgets += 1
                    if option == 'manual' and name == 'custom_partitioning':
                        opt_widget.set_active(True)
                    else:
                        opt_widget.set_active(False)
            else:
                opt_widget.hide()
                opt_desc.hide()
        ...
        # kobe
        if len_widgets <= 2:
            self.expander1.set_visible(False)
            len_widgets = 0
        else:
            self.expander1.set_visible(True)
            len_widgets = 0
        # kobe
        self.use_crypto.set_visible(True)
        self.use_crypto_desc.set_visible(True)
        self.use_lvm.set_visible(True)
        self.use_lvm_desc.set_visible(True)
        ...
        if not os.path.exists("/rofs/usr/bin/kybackup"):
            ...
            self.backup_checkbutton_factory.hide()
            self.backuplabel_factory.hide()
            self.backup_checkbutton_factory.set_active(False)
    ...
    def partman_dialog(self, devpart, partition, create=True):
        ...
        if create:
            ...
            for script, arg, option in partition['method_choices']:
                # kobe
                # if arg in ('ext4', 'swap'):
                if arg in ('ext4', 'swap', 'efi', 'dontuse'):
    ...
    def update_partman(self, disk_cache, partition_cache, cache_order):
        ...
        for item in cache_order:
            # kylin disable show docker disk
...
class Page(plugin.Plugin):
    ...
    def default_mountpoint_choices(self, fs='ext4'):
        ...
        for i in range(len(choices_c)):
            ...
            elif choices_c[i].startswith('/boot - '):
                yield (choices_c[i].split(' ')[0], choices_c[i], choices[i])
            elif choices_c[i].startswith('/data'):
                yield (choices_c[i].split(' ')[0], choices_c[i], choices[i])
            elif choices_c[i].startswith('/backup'):
                yield (choices_c[i].split(' ')[0], choices_c[i], choices[i])
    ...
    def get_contents(self):
        ...
        q = 'ubiquity/partitioner/factory'
        title = self.description(q)
        desc = self.extended_description(q)
        contents['factory'] = PartitioningOption(title, desc)
        return contents
